...
import { createBookingSchema, listBookingsQuerySchema, adminListBookingsQuerySchema } from '../schemas/bookingSchemas.js'
...
export async function cancelOwnBookingPost(req: Request, res: Response) {
    const userId = req.user!.id
    const bookingId = req.params.id as string

    try {
        const result = await bookingService.cancelBooking(userId, bookingId, false)
        res.json(result)
    } catch (err) {
        if (err instanceof AppError) return res.status(err.statusCode).json({ error: err.message })
        console.error(err)
...
export async function adminCancelBookingPost(req: Request, res: Response) {
    const bookingId = req.params.id as string

    try {
        const result = await bookingService.cancelBooking(req.user!.id, bookingId, true)
        res.json(result)
    } catch (err) {
        if (err instanceof AppError) return res.status(err.statusCode).json({ error: err.message })
        console.error(err)
...
export async function adminGetAllBookings(req: Request, res: Response) {
    const result = adminListBookingsQuerySchema.safeParse(req.query)
    if (!result.success) return res.status(400).json({ error: 'Invalid query parameters', details: result.error.flatten() })

    try {
        const { data, pagination } = await bookingService.getAllBookings(result.data)
        res.json({ data, pagination })
    } catch (err) {
        if (err instanceof AppError) return res.status(err.statusCode).json({ error: err.message })
        console.error(err)
